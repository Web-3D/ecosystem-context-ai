...
| `shared-gltf-pipeline`      | ⚠️ Một phần   | Pipeline order (weld→simplify→normals→draco) — gltf-transform docs ✅. `GLTFLoader.setDRACOLoader()` ✅. `DRACOLoader.setDecoderPath()` ✅                     | "draco giảm 60–80%" — typical industry number, chưa đo trong project                      | Chạy pipeline trên 1 asset thực, đo trước/sau     |
